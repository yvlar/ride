...
Pour la recherche unifiée et le géocodage inverse, `GEOCODING_PROVIDER=mock` reste la valeur locale et de test. Un adaptateur Nominatim compatible (`GEOCODING_PROVIDER=nominatim`) peut être activé en définissant `GEOCODING_API_BASE_URL` vers un service dédié ou géré. `GEOCODING_API_KEY` est facultative. L’API publique `nominatim.openstreetmap.org` est refusée explicitement, car sa politique interdit l’autocomplétion. Les appels passent uniquement par le serveur, derrière `GeocodingProvider.search()` et `GeocodingProvider.reverse()`.
