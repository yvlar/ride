...
L’application démarre sur [http://localhost:3000](http://localhost:3000). Sans clés externes, l’adaptateur de routage RAG (`ROUTING_PROVIDER=ai-rag`) utilise un corpus de corridors en mémoire. `ROUTING_PROVIDER=mock` reste disponible pour un graphe simulé isolé.

Le projet Vercel `yvlars-projects/ride` est lié au dépôt GitHub. Les poussées vers `main` déploient la production; les autres branches créent des previews. Aucune clé de routage n’est requise pour `ai-rag`. Un moteur de graphe ou un modèle distant peut remplacer cet adaptateur plus tard via la même interface.
