...
Pour suivre les routes OpenStreetMap réelles, définir `ROUTING_PROVIDER=osrm` et `ROUTING_API_BASE_URL` vers une instance OSRM dédiée ou gérée dans chaque environnement Vercel visé. Le serveur public de démonstration n’est jamais configuré par défaut. `ROUTING_PROVIDER=ai-rag` et l’option **Corridors RAG** (`FR-029`) classent des corridors via ChatGPT sur le graphe local indexé, puis collent le tracé sur l’adaptateur de réseau configuré (OSRM lorsqu’il est branché ; sinon géométrie simulée). Définir `OPENAI_API_KEY` côté serveur (jamais `NEXT_PUBLIC_`). L’option du formulaire demande ce pipeline pour une génération, sans modifier `ROUTING_PROVIDER`.
