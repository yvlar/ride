...
Pour suivre les routes OpenStreetMap réelles, définir `ROUTING_PROVIDER=osrm` et `ROUTING_API_BASE_URL` vers une instance OSRM dédiée ou gérée dans chaque environnement Vercel visé. Le serveur public de démonstration n’est jamais configuré par défaut. `ROUTING_PROVIDER=ai-rag` et l’option **Corridors RAG** (`FR-029`) de l’écran principal utilisent le graphe local indexé, classé par ChatGPT. Ce n’est pas un réseau OSM. Définir `OPENAI_API_KEY` côté serveur (jamais `NEXT_PUBLIC_`). L’option du formulaire demande cet adaptateur pour une génération, sans modifier `ROUTING_PROVIDER`.
