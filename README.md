...
- intégrations de navigation externes;
- profils de motos;
...
L’application démarre sur [http://localhost:3000](http://localhost:3000). Le fichier `.env.example` conserve `ROUTING_PROVIDER=mock` afin que le démarrage local et les tests ne sollicitent aucun service de routage partagé. Le géocodage, lui, est branché par défaut sur un service public **sans clé** (`GEOCODING_PROVIDER=photon`), de sorte que la recherche d’adresse fonctionne sans configuration; `GEOCODING_PROVIDER=mock` la ramène aux quelques lieux de démonstration, hors réseau.

Pour suivre les routes OpenStreetMap réelles, définir `ROUTING_PROVIDER=osrm` et `ROUTING_API_BASE_URL` vers une instance OSRM dédiée ou gérée dans chaque environnement Vercel visé. Le serveur public de démonstration n’est jamais configuré par défaut. `ROUTING_PROVIDER=ai-rag` et l’option **Corridors RAG** (`FR-029`) classent des corridors via ChatGPT sur le graphe local indexé, puis collent le tracé sur l’adaptateur de réseau configuré (OSRM lorsqu’il est branché ; sinon géométrie simulée). Définir `OPENAI_API_KEY` côté serveur (jamais `NEXT_PUBLIC_`). L’option du formulaire demande ce pipeline pour une génération, sans modifier `ROUTING_PROVIDER`. **Décrire mon trajet** (`FR-034`) utilise cette même clé pour la recherche Web si `WEB_SEARCH_API_KEY` (Tavily ou Brave) n’est pas défini.

La recherche d’adresse et le géocodage inverse de « Ma position » passent par défaut par **Photon** (`GEOCODING_PROVIDER=photon`), un géocodeur public sans clé conçu pour l’autocomplétion : il répond sur une saisie partielle (« drummondv », « 722 rue des boul roxton »), ce qu’un géocodeur strict ne fait pas. `GEOCODING_API_BASE_URL` vide vise l’instance publique `https://photon.komoot.io/` — elle est offerte pour un **usage équitable**, donc un trafic soutenu doit viser une instance auto-hébergée par cette même variable. `GEOCODING_PROVIDER=nominatim` reste disponible pour une instance Nominatim dédiée (`GEOCODING_API_BASE_URL` obligatoire) ; son serveur public de démonstration interdit l’autocomplétion et n’est jamais configuré ici. `GEOCODING_PROVIDER=mock` donne des lieux déterministes hors ligne. `GEOCODING_API_KEY` est facultative. Les appels passent uniquement par le serveur, via `GeocodingProvider`.
...
La couche **Météo** de la carte (`FR-043`) est allumée par le bouton du même nom et n’émet aucune requête tant qu’elle est éteinte. Ses deux fournisseurs sont publics et **sans clé**, donc branchés par défaut : `WEATHER_PROVIDER=open-meteo` pour les prévisions (nuages, probabilité de pluie, direction à éviter) et `RADAR_PROVIDER=rainviewer` pour les images radar.

Deux services radar sont branchés. `rainviewer` couvre le monde entier et offre une prévision immédiate, mais ses tuiles publiques s’arrêtent au zoom 7 : la carte doit les agrandir au zoom de roulage. `RADAR_PROVIDER=geomet` bascule sur le service GeoMet du Service météorologique du Canada — composite nord-américain à 1 km, rendu à la demande sur n’importe quelle emprise, donc net en roulant ; en contrepartie il ne fournit que des observations (pas de prévision immédiate) et reste transparent hors Amérique du Nord. Les deux sont publics et sans clé. `WEATHER_API_BASE_URL` permet de viser une instance Open-Meteo dédiée, `WEATHER_API_KEY` et `RADAR_API_KEY` restent facultatives et côté serveur. `WEATHER_PROVIDER=mock` et `RADAR_PROVIDER=mock` donnent un ciel déterministe hors ligne pour le développement et les tests. Les appels passent uniquement par `/api/weather`.
...
- Le biais de proximité (`lat`/`lon` pour Photon, `viewbox` sans `bounded` pour Nominatim) n'est qu'une **préférence**, jamais un filtre : une destination à l'autre bout de la province revient quand même. D'où le reclassement (`Québec`, puis `Canada`, puis distance) effectué côté serveur dans le domaine.
- Photon rapproche les codes postaux de façon **floue** : interrogé pour `J2G 2W4`, il propose volontiers `J2C 2W8`, une autre ville. L'adaptateur ne garde donc que le code exactement demandé, puis se rabat sur la RTA plutôt que d'afficher un code faux.
- Un fournisseur répond avec les objets qu'il indexe, pas avec des choix : une rue arrive en autant de lignes que de segments OSM. Les doublons sont fusionnés côté serveur **après** le reclassement, de sorte que la ligne conservée est le segment le plus proche du motard.
- `countrycodes` (Nominatim) est un filtre **exclusif**. Il n'est donc pas utilisé pour « prioriser » le Canada, ce qui bloquerait les destinations étrangères.
...
L’aperçu et la navigation partagent une seule carte routière. Démarrer la navigation agrandit cette carte et passe en vue 3D cap-en-haut (`FR-024`) au lieu d’en monter une seconde, ce qui évite le plantage mémoire iOS suivi dans [MapLibre GL JS #7667](https://github.com/maplibre/maplibre-gl-js/issues/7667) tout en gardant le tracé aligné sur les rues (`FR-013`). Le repli raster OSM reste plat ; un `NEXT_PUBLIC_MAP_STYLE_URL` vectoriel avec une couche `building` affiche les volumes. Le fond de carte se choisit dans **Réglages** (`FR-045`) — Automatique, Clair, Sombre, Satellite, Relief ou **Kart Arcade** — et se change sans démonter la carte ; `NEXT_PUBLIC_MAP_STYLE_URL` définit le fond du thème **Clair**, et les thèmes matriciels Satellite et Relief n’affichent pas de volumes. **Kart Arcade** (`FR-046`) est un style MapLibre vectoriel complet et original : terrain vert, eau turquoise, routes dorées et corail, bâtiments orangés, tracé bleu électrique à halo blanc. Il lit ses tuiles par `NEXT_PUBLIC_MAP_VECTOR_TILES_URL` (aucune clé dans le code), garde l’attribution OpenStreetMap, incline la carte d’exploration à 45° pour la profondeur, allège son décor pendant la navigation, et revient de lui-même au thème par défaut si son fond ne se charge pas. Sous cette inclinaison, un ciel dégradé ferme l’horizon et les bâtiments sortent en volumes dès le zoom 13,5 ; le tracé reçoit un damier de départ et d’arrivée orienté par le cap réel de la route, et des bornes kilométriques entre les deux, masquées pendant la navigation. Le thème s’applique aussi à l’interface : le fond choisi est publié sur le document, et panneaux, contrôles et boutons prennent le contour épais et l’ombre dure du thème. `NEXT_PUBLIC_MAP_TERRAIN_TILES_URL` ajoute en option un ombrage de relief ; son absence n’est pas une erreur (`NFR-005`). Au premier point GPS d’une navigation, un compte à rebours 3‑2‑1‑GO ! ponctuel s’affiche au centre, sonorisé et coupé par le bouton Muet ; il est purement décoratif et ne retarde ni le guidage ni la voix. Les indications sonores (`FR-044`) prennent par ailleurs un timbre chiptune sous ce thème — elles restent un repli, donc on ne les entend que si la synthèse vocale est indisponible.
