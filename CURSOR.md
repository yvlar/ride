...
`NEXT_PUBLIC_MAP_STYLE_URL` définit le fond du thème de carte **Clair** (`FR-045`), celui que **Automatique** utilise en apparence claire; les thèmes **Sombre**, **Satellite**, **Relief** et **Kart Arcade** sont intégrés et l’ignorent. Laisser la variable vide utilise le repli raster OSM (`FR-013`, `NFR-005`). Un style vectoriel avec une couche `building` active les bâtiments 3D pendant le suivi de navigation (`FR-024`). Exemple sans secret : `https://tiles.openfreemap.org/styles/liberty`.

Le thème **Kart Arcade** (`FR-046`) construit son propre style MapLibre vectoriel et lit trois variables publiques, toutes facultatives et sans clé : `NEXT_PUBLIC_MAP_VECTOR_TILES_URL` (TileJSON au schéma OpenMapTiles; vide vise `https://tiles.openfreemap.org/planet`), `NEXT_PUBLIC_MAP_GLYPHS_URL` (polices; vide vise les polices du même hôte) et `NEXT_PUBLIC_MAP_TERRAIN_TILES_URL` (élévation encodée Terrarium; vide signifie simplement aucun ombrage de relief). Les points d’accès publics sont offerts pour un usage équitable : un trafic soutenu doit viser une instance dédiée par ces mêmes variables. L’attribution OpenStreetMap accompagne la source quelle que soit la configuration.
