...
Chaque point non dégagé porte un nuage dont la teinte et les traits suivent le niveau — **nuageux**, **averses**, **pluie**, **orage** — déduit de la probabilité de précipitation, de l’intensité en mm/h, de la couverture nuageuse et du code orage du fournisseur. Le pourcentage de risque **n’est pas affiché sur la carte** : le niveau se lit sur le dessin — teinte, traits, humeur — et **aucune information n’est portée par la seule couleur**, chaque marqueur portant un nom accessible qui dit le niveau et le risque en toutes lettres (« Pluie, 72 % de risque de pluie »). Un ciel dégagé ne reçoit aucun marqueur.

Le nuage est un **personnage**, sur tous les thèmes de carte : un corps plein cerné d’encre, deux yeux, et une **humeur qui suit le niveau** — placide sous un ciel couvert, inquiet sous les averses, triste sous la pluie, fâché sous l’orage. Il est dessiné **assez grand** pour se lire d’un coup d’œil à travers une visière, et **translucide** : la rue, la route et les repères restent lisibles sous lui, sans que sa teinte ne change — le niveau garde exactement sa couleur. Le marqueur reste transparent aux gestes, et il n’intercepte donc jamais un déplacement de la carte. L’humeur ne fait que **redire** ce que le niveau dit déjà : le nom accessible porte l’information (`NFR-001`), et le dessin reste `aria-hidden`.

À cette taille, deux nuages voisins se recouvriraient sur un cadrage régional. Ils **fusionnent** alors en un **seul nuage plus grand**, dont la taille croît avec le nombre de points qu’il représente (bornée, pour qu’une fusion ne recouvre jamais la carte). La fusion est reprise tant qu’il reste un recouvrement, de sorte qu’aucun visage n’en cache un autre. Deux nuages voisins ne sont jamais dessinés exactement à la même taille : chacun s’écarte un peu de la taille de base, **toujours du même écart** — la taille se déduit de l’identité du nuage, jamais d’un tirage au sort, de sorte qu’un zoom, un changement de thème ou un rafraîchissement redessine le même ciel au lieu de le rebattre. L’écart reste plus petit que ce qu’une fusion ajoute : un nuage seul n’est jamais dessiné plus gros qu’un nuage fusionné. Le nuage fusionné porte le **pire** niveau et le **plus haut** risque des points réunis — jamais une moyenne, qui adoucirait un orage — et son nom accessible dit le nombre de zones réunies (« Pluie, 72 % de risque de pluie, 4 zones regroupées ») : la taille ne fait que redire ce que le texte porte déjà (`NFR-001`). Le recouvrement se mesure en **pixels**, donc au zoom courant : en s’approchant, les nuages se séparent de nouveau, chacun reprenant son point.
...
Le rendu conserve une densité de quatre nuages maximum par tuile de 256 pixels, chacun d’une taille propre et stable comme ceux des prévisions. **Aucun alignement** : la cellule d’échantillonnage dit s’il y a un nuage et de quelle couleur, mais la place du nuage dans la cellule est tirée de son identité, jamais son centre — le ciel ne doit pas se lire en rangées et en colonnes. Ni la donnée ni le hasard ne peuvent y remettre la grille : la position est stable d’un rendu à l’autre, et une averse uniforme, qui donne le même écho partout, reste dispersée. Chaque nuage tient entier dans sa tuile, faute de quoi une couture apparaîtrait avec la tuile voisine. Les silhouettes sont redessinées à chaque niveau de zoom. Au-delà du zoom fourni par GeoMet ou RainViewer, il utilise la portion correspondante de la dernière tuile disponible; il ne demande jamais un niveau non pris en charge au fournisseur. Les téléchargements de tuiles parentes sont partagés et annulés lorsqu’ils n’ont plus de consommateur, avec un cache limité à seize images. Les autres thèmes conservent leur imagerie radar habituelle. Une panne du radar ne déclenche pas un changement de thème et n’interrompt pas la navigation.
