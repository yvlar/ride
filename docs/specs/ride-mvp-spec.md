...
À cette taille, deux nuages voisins se recouvriraient sur un cadrage régional. Ils **fusionnent** alors en un **seul nuage plus grand**, dont la taille croît avec le nombre de points qu’il représente (bornée, pour qu’une fusion ne recouvre jamais la carte). La fusion est reprise tant qu’il reste un recouvrement, de sorte qu’aucun visage n’en cache un autre. Le nuage fusionné porte le **pire** niveau et le **plus haut** risque des points réunis — jamais une moyenne, qui adoucirait un orage — et son nom accessible dit le nombre de zones réunies (« Pluie, 72 % de risque de pluie, 4 zones regroupées ») : la taille ne fait que redire ce que le texte porte déjà (`NFR-001`). Le recouvrement se mesure en **pixels**, donc au zoom courant : en s’approchant, les nuages se séparent de nouveau, chacun reprenant son point.
