...
/**
 * Route-only navigation map that uses SVG instead of WebGL. It is deliberately
 * small and static so GPS guidance remains usable when iOS cannot safely create
 * another MapLibre context after the route preview.
 */
export function createLightweightNavigationMapEngine(): MapEngine {
  return {
    mount(container, viewModel) {
      let disposed = false;
      ...
      route.setAttribute("points", routePoints(viewModel));
      svg.append(route);

      appendPlaceMarker(
        svg,
        project(viewModel.start.coordinates, viewModel),
        "Départ",
        "#16a34a",
      );
      if (viewModel.destination) {
        appendPlaceMarker(
          svg,
          project(viewModel.destination.coordinates, viewModel),
          "Destination",
          "#dc2626",
        );
      }
      ...
      userMarker.append(halo, dot);
      svg.append(userMarker);

      const notice = document.createElement("p");
      notice.className =
        "absolute left-3 top-3 rounded-md bg-background/90 px-2 py-1 text-xs text-foreground shadow-sm";
      notice.textContent = "Carte allégée · stabilité iOS";
      ...
      function updateFollowView() {
        ...
        const offset = FOLLOW_VIEW_SIZE / 2;
        svg.setAttribute(
          "viewBox",
          `${userPoint.x - offset} ${userPoint.y - offset} ${FOLLOW_VIEW_SIZE} ${FOLLOW_VIEW_SIZE}`,
        );
      }
...
function project(
  ...
  return {
    x:
      PADDING +
      ((coordinates.longitude - viewModel.bounds.west) / longitudeSpan) * drawable,
    y:
      PADDING +
      ((viewModel.bounds.north - coordinates.latitude) / latitudeSpan) * drawable,
  };
}
