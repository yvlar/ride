...
export async function POST(request: Request) {
  ...
  if (Number.isFinite(contentLength) && contentLength > MAX_BODY_BYTES) {
    return errorResponse(
      requestId,
      {
        ...
        suggestions: [
          "Envoyez uniquement la demande et la géométrie du trajet précédent.",
        ],
      },
      400,
    );
  ...
  try {
    ...
    return errorResponse(
      requestId,
      {
        ...
        suggestions: [
          "Envoyez un objet JSON avec request et previousRoute.geometry (FR-012).",
        ],
      },
      400,
    );
  ...
  try {
    const result = await regenerateRide(body);
    if (!result.ok) {
      const status =
