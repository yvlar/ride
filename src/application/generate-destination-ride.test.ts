...
describe("generateDestinationRide (FR-002)", () => {
  ...
  it("keeps explicit distance as the length constraint and flags a duration ceiling miss (FR-010)", async () => {
    const slowProvider: RoutingProvider = {
      async calculateRoute(
        ...
        return { ...routed, durationMinutes: 300 };
      },
    };
    ...
    expect(result.route.targetDistanceKm).toBe(240);
    expect(isWithinDistanceTolerance(result.route.distanceKm, 240)).toBe(true);
    expect(result.route.warnings).toContain(
      availableDurationCeilingWarning(300, 90),
    );
  });
  ...
  it("selects a paved corridor when avoidUnpaved is on (FR-008)", async () => {
    const mock = new MockRoutingProvider();
    const provider: RoutingProvider = {
      async calculateRoute(input) {
        const routed = await mock.calculateRoute(input);
  ...
  it("warns when the selected destination includes an unknown surface (FR-008)", async () => {
    const mock = new MockRoutingProvider();
    const provider: RoutingProvider = {
      async calculateRoute(input) {
        const routed = await mock.calculateRoute(input);
  ...
  it("selects a Canadian corridor when stayInCanada is on (FR-028)", async () => {
    const mock = new MockRoutingProvider();
    const provider: RoutingProvider = {
      async calculateRoute(input) {
        const routed = await mock.calculateRoute(input);
  ...
  it("rejects every United States corridor (FR-028, BR-009)", async () => {
    const mock = new MockRoutingProvider();
    const provider: RoutingProvider = {
      async calculateRoute(input) {
        const routed = await mock.calculateRoute(input);
  ...
  it("fails before routing when the destination is in the United States (FR-028, FR-021)", async () => {
